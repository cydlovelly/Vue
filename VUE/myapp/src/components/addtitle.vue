<template>
  <div class="addtitle">
    <form>
      ...
      <input type="button" class="btn btn-success" value="发布" v-on:click="addlist" />
  ...
  data () {
    return {
      list:{},
       ...
       headportrait:[
            {con:"阳光",value:require("@/assets/yangguang.jpg")},
            {con:"彭于晏",value:require("@/assets/pengyuyan.jpg")},
            {con:"沙雕",value:require("@/assets/shadiao.jpg")},
            {con:"杀马特",value:require("@/assets/shamate.jpg")},
            {con:"黄色",value:require("@/assets/huangse.jpg")}
          ],
           picture:[
            {text:"帅气",value:require("@/assets/suaiqi.jpg")},
            {text:"炫酷",value:require("@/assets/xuanku.jpg")},
            {text:"伤感",value:require("@/assets/shanggan.jpg")},
            {text:"非主流",value:require("@/assets/feizhuliu2.jpg")},
            {text:"福利",value:require("@/assets/fuli.jpg")}
          ],
    }
  },
  methods:{
    addlist(e){
      if(!this.list.name || !this.list.conent){
        ...
        this.$http.post("http://localhost:3000/users",datalist)
        .then(function(response){
            this.$router.push({path:"/"},alert("段子添加成功!"))
        })
      }   
    }
  }
